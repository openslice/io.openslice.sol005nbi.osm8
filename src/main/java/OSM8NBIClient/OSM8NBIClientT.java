...
import org.opendaylight.yang.gen.v1.urn.etsi.osm.yang.mano.types.rev170208.ip.profile.info.IpProfileParamsBuilder;
import org.opendaylight.yang.gen.v1.urn.etsi.osm.yang.mano.types.rev170208.ip.profile.info.ip.profile.params.DnsServer;
import org.opendaylight.yang.gen.v1.urn.etsi.osm.yang.mano.types.rev170208.ip.profile.info.ip.profile.params.DnsServerBuilder;
...
import OSM8NBIClient.NSInstantiateInstanceRequestPayload.NInterface;
import OSM8NBIClient.NSInstantiateInstanceRequestPayload.Vdu;

public class OSM8NBIClientT {
	
	public static void main(String args[]) {
	    //OSM8Client OSM8Client = new OSM8Client("https://192.168.199.115:9999","admin","admin","admin");
	    OSM8Client OSM8Client = new OSM8Client("https://10.10.10.37:9999","admin","admin","admin");
	    ...
	    System.out.println("************************");
	    OSM8Client.getVNFPackages();		
	    System.out.println("************************");
	    System.out.println("Getting VNFDs");
	    System.out.println("************************");
		Vnfd[] vnfds = OSM8Client.getVNFDs();
		if(vnfds!=null)
		{
			for (Vnfd v : vnfds) {
	...
	//			System.out.println("=== LIST VNFDs POJO object id: " + tmp_vnfd.getId() + ", Name: " + tmp_vnfd.getName());			
	    ...
	    System.out.println("************************");
	    OSM8Client.getNSDescriptors();		
	    System.out.println("************************");
	    System.out.println("Getting NSDs");
	    System.out.println("************************");
		Nsd[] nsds = OSM8Client.getNSDs();
		for (Nsd v : nsds) {
...
//			System.out.println("=== LIST VNFDs POJO object id: " + tmp_nsd.getId() + ", Name: " + tmp_nsd.getName());			
		...
		response = OSM8Client.createVNFDPackage();
		...
		else
		{
			...
			try {
				response = OSM8Client.uploadVNFDPackageContent(vnfd_id, vnfd_package_path);
				if (response == null || response.getStatusCode().is4xxClientError()
					...
					OSM8Client.deleteVNFDPackage(vnfd_id);
					return;
				}
				else
				{
...
//		}
	    ...
	    String nsd_id = null;
		response = OSM8Client.createNSDPackage();
		if (response == null || response.getStatusCode().is4xxClientError() || response.getStatusCode().is5xxServerError()) {
		...
		else
		{
		    ...
		    System.out.println("************************");
			try {
				response = OSM8Client.uploadNSDPackageContent(nsd_id, nsd_package_path);
				if (response == null || response.getStatusCode().is4xxClientError()
						|| response.getStatusCode().is5xxServerError()) {
					System.out.println("Upload of NSD Package Content failed. Deleting NSD Package.");
					// Delete the package from the OSM
					OSM8Client.deleteVNFDPackage(nsd_id);
					return;
				}
				else
				{
...
//		}
		...
		List<ConstituentVnfd> constituentVnfds = OSM8Client.getNSDbyID(nsd_id).getConstituentVnfd();
		...
		{
			OSM8NBIClient.NSInstantiateInstanceRequestPayload.VnF vnf_tmp = nscreateinstancerequestpayload.new VnF();  
			count=count+1;
			...
			else
			{
				...
				OSM8NBIClient.NSInstantiateInstanceRequestPayload.Vdu vdu_tmp = nscreateinstancerequestpayload.new Vdu();
				vdu_tmp.setId("cirros_vnfd-VM");
				OSM8NBIClient.NSInstantiateInstanceRequestPayload.NInterface interface_tmp = nscreateinstancerequestpayload.new NInterface();
		...
		{
			OSM8NBIClient.NSInstantiateInstanceRequestPayload.Vld vld_tmp = nscreateinstancerequestpayload.new Vld();  
...
//			vld_tmp.setIpProfile(ipprofileparams_tmp);
			...
			OSM8NBIClient.NSInstantiateInstanceRequestPayload.Vld vld_tmp2 = nscreateinstancerequestpayload.new Vld();  
		...
		ResponseEntity<String> ns_instance_creation_entity = OSM8Client.createNSInstance(nscreateinstancerequestpayload.toJSON());
		...
		if (ns_instance_creation_entity == null || ns_instance_creation_entity.getStatusCode().is4xxClientError()
			...
			for(ConstituentVnfd constituenVnfd : constituentVnfds)
			{
				OSM8NBIClient.NSInstantiateInstanceRequestPayload.VnF vnf_tmp = nsrequestpayload.new VnF();  
				count=count+1;
				vnf_tmp.setMemberVnFIndex(count.toString());
				if(count!=2)
					vnf_tmp.setVimAccount("4efd8bf4-5292-4634-87b7-7b3d49108b36");
			...
			{
				OSM8NBIClient.NSInstantiateInstanceRequestPayload.Vld vld_tmp = nsrequestpayload.new Vld();  
				vld_tmp.setName("cirros_3vnf_nsd_vld1");
...
//				vld_tmp.setIpProfile(ipprofileparams_tmp);
				...
				OSM8NBIClient.NSInstantiateInstanceRequestPayload.Vld vld_tmp2 = nsrequestpayload.new Vld();  
				vld_tmp2.setName("cirros_3vnf_nsd_vld2");
				//vld_tmp2.setVimNetworkName("OSM_mgmt_net");
				LinkedHashMap<String,String> tmp2=new LinkedHashMap<>();
				tmp2.put("4efd8bf4-5292-4634-87b7-7b3d49108b36","OSM_mgmt_net");
			...
			ResponseEntity<String> instantiate_ns_instance_entity = OSM8Client.instantiateNSInstance(nsd_instance_id, nsrequestpayload.toJSON());
